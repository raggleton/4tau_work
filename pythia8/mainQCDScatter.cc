...
int main(int argc, char* argv[]) {
  ...
  while(iEvent < nEvent) {
    ...
    nMuInEventNoHLT.fill(nMuPos + nMuNeg);
    ...
    if (wantedHLT && outputEvent && iEvent <= 1)
      event.list();
  ...
  pythia.stat();
  cout << muPt << nMuInEventNoHLT << nMuInEvent << muPtNoHLT << endl;
  cout << "Number of events with pair of muon: " << nWithMuPair << endl;
